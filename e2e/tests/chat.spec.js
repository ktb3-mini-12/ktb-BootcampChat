    ...
    test("여러 메시지 연속 전송", async ({ page }) => {
      ...
      await Promise.all(
        messages.map((message) =>
          expect(page.getByTestId("message-content").filter({ hasText: message })).toBeVisible()
        )
      );
    });
  });
  ...
  test.describe("실시간 채팅", () => {
    test("다자간(5인) 실시간 메시지 송수신", async ({ browser }) => {
      ...
      for (const [index, senderPage] of allPages.entries()) {
        ...
        await Promise.all(
          allPages.map((page) =>
            expect(page.getByTestId("message-content").filter({ hasText: message })).toBeVisible({ timeout: 15000 })
          )
        );
      }
